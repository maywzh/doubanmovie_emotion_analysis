...
class GLDA(object):
    ...
    def lda_train(self, num_topics, datafolder, middatafolder, dictionary_path=None, corpus_path=None, iterations=5000, passes=1, workers=3):
        ...
        num_docs = 0
        doclist = []
        # 若无字典或无corpus，则读取预处理后的docword。一般第一次运行都需要读取，在后期调参时，可直接传入字典与corpus路径
        ...
        if corpus_path:
            ...
            corpora.MmCorpus.serialize(
                middatafolder + 'corpus.mm', corpus)  # 保存corpus
        ...
        print('lda training time cost is : %s, all time cost is : %s ' %
              (datetime.now()-time2, datetime.now()-time1))
        #模型的保存/ 加载
        lda_multi.save(middatafolder + 'lda_tfidf_%s_%s.model' %
                       (num_topics, iterations))  # 保存模型
...
if __name__ == '__main__':
    datafolder = path + os.sep + 'docword' + \
        os.sep  # 预处理后的语料所在文件夹，函数会读取此文件夹下的所有语料文件
    ...
    workers = 3  # 相当于进程数
    lda = GLDA()
    lda.lda_train(num_topics, datafolder, middatafolder, dictionary_path=dictionary_path,
                  corpus_path=corpus_path, iterations=iterations, passes=passes, workers=workers)
